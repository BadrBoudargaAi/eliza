    ...
    implements IDatabaseCacheAdapter
{
    ...
    async getCachedEmbeddings(opts: {
        ...
        // Check if input exceeds 255 characters and log a warning if needed
        let truncatedInput = opts.query_input;
        if (opts.query_input.length > 255) {
            elizaLogger.warn("query_input exceeds 255 characters, truncating:", {
                originalLength: opts.query_input.length,
                truncatedLength: 255,
                tableName: opts.query_table_name
            });
            truncatedInput = opts.query_input.slice(0, 255);
        }
